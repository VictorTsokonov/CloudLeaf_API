...
public class StatusController {
	...
	@PostMapping("/status")
	public CompletableFuture<List<Object>> checkStatus(
		...
		scheduledFuture[0] = executorService.scheduleAtFixedRate(() -> {
			if (retryCount.incrementAndGet() > 240) { // Limit to 240 retries i.e., around
														// 20 minutes.
				completableFuture.completeExceptionally(
						new Exception("Exceeded maximum retry attempts"));
			...
			try {
				...
				ResponseEntity<String> response = restTemplate.getForEntity(
						"http://" + statusRequest.getIpAddressAndPort(), String.class);

				if (response.getStatusCode() == HttpStatusCode.valueOf(200)) {
					long timeToConnect = System.currentTimeMillis() - startTime;
					completableFuture.complete(Arrays
							.asList(statusRequest.getIpAddressAndPort(), timeToConnect));

					Optional<RepoEntity> original = repoService
							.getRepoByName(statusRequest.getRepoName());
					if (original.isEmpty()) {
						throw new RuntimeException("The repo with this name: "
								+ statusRequest.getRepoName() + " doesn't exist");
					}

					// Check the status of the original RepoEntity
					if (!original.get().status().equals("Terminated")) {
						RepoEntity updatedRepo = new RepoEntity(original.get().repoId(),
								original.get().userId(), original.get().repoName(),
								original.get().cloneUrl(), original.get().sshUrl(),
								original.get().ec2InstanceId(),
								statusRequest.getIpAddressAndPort(), "Deployed");
						repoService.updateRepo(updatedRepo);
					}
					System.out.println("FETCHING FOR STATUS SHOULD BE STOPPED NOW");
					scheduledFuture[0].cancel(false); // Stop the task once the status
														// code is 200
				}
			}
			catch (Exception ignored) {
				// If we can't connect, just ignore the exception and try again in 5
				// seconds
			}
