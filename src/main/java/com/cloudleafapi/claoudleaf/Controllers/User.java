...
public class User {
    ...
    @PostMapping
    public UserEntity createUser(@RequestParam String github_username, @RequestParam String github_access_token) {
    ...
    public UserEntity getUser(@PathVariable UUID id) {
        return userService.getUser(id).orElseThrow(() -> new IllegalArgumentException("User not found"));
    }
    ...
    public ResponseEntity<UserEntity> updateUser(@PathVariable UUID userId,
        ...
        UserEntity currentUser = userService.getUser(userId).orElseThrow(() -> new RuntimeException("User not found"));
        currentUser = new UserEntity(userId, userEntity.github_username(), userEntity.github_access_token());
